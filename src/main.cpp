...
double tetrahedra_volume(const CGAL::Epick::Kernel::Point_3& A, const CGAL::Epick::Kernel::Point_3& B, const CGAL::Epick::Kernel::Point_3& C, const CGAL::Epick::Kernel::Point_3& D) {
    // Calculate vectors AB, AC, and AD
    CGAL::Epick::Kernel::Vector_3 AB(A, B);
    CGAL::Epick::Kernel::Vector_3 AC(A, C);
    CGAL::Epick::Kernel::Vector_3 AD(A, D);

    // Calculate cross product AC x AD
    CGAL::Epick::Kernel::Vector_3 cross = CGAL::cross_product(AC, AD);

    // Calculate dot product of AB and cross product
    double volume = CGAL::to_double(AB * cross);

    // Divide by 6 to get volume of tetrahedron
    return volume/6.0;
}
...
std::pair<double, double> SurfArea_Volume_Object(std::map<int, Face>& faces, std::map<int, Vertex>& vertices) {
    ...
    vector_out.y = 0;
    vector_out.z = 0;
    Point3 point_out (vector_out.x, vector_out.y, vector_out.z);
...
//                  << f.tri_vertices[2] << std::endl;
        ...
        double face_area=0, face_vol=0;
        face_area = Area_Calc(f, vertices);
        face_vol = tetrahedra_volume(f.v3_coors.at(0), f.v3_coors.at(1), f.v3_coors.at(2), point_out);
...
//    double surface_area = 0, volume = 0;
//    int number_of_triangles = faces.size();
//    for (auto& )
...
double OOBB_Volume(double& volume_object, double& surface_area_object, std::map<int, Face>& faces, std::map<int, Vertex>& vertices) {
    std::vector<int> checker;
    std::vector<Point3> object_points_3d;
    for (auto& [key, face] : faces) {
        int tv0, tv1, tv2;  // triangle vertices ids
...
int main(int argc, const char * argv[]) {
    ...
    vertices = get_coordinates(j, false);
...
//                            std::cout << "f " << (tr[0] + 1) << " " << (tr[1] + 1) << " " << (tr[2] + 1) << std::endl;
                ...
                double oobb_volume_object = OOBB_Volume(volume_object, surface_area_object, faces, vertices);
                ...
                j["CityObjects"][parent_obj_name]["attributes"]["SurfaceArea"] = surface_area_object;
                ...
                j["CityObjects"][parent_obj_name]["attributes"]["OOBB_Volume"] = oobb_volume_object;
